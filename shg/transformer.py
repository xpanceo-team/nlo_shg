...
class Output_block(nn.Module):
    ...
    def forward(self, node_feature):
        ...
        for i in range(3):
            ...
            grad_outputs[:, i] = 1.0
            grad_D = grad(D_, outer_S, grad_outputs=grad_outputs, create_graph=True, retain_graph=True)[0].reshape(bs, 9)

            target.append(grad_D[:, [0, 4, 8, 1, 5, 6]])
        target = torch.stack(target).transpose(0, 1)
        target = self.output_linear(target.reshape(bs, 18))
        return target.reshape(bs, 3, 6)
